...
if ! printf '%s' "$PORT" | grep -Eq '^[0-9]+$'; then
  echo "拒绝：端口不是合法整数（${PORT}）"
  exit 1
fi
PORT_NUM="$(printf '%s' "$PORT" | sed 's/^0*//')"
PORT_NUM="${PORT_NUM:-0}"
if (( PORT_NUM == 8765 )); then echo "拒绝：端口 8765 是真实实例保留端口"; exit 1; fi
mkdir -p "$(dirname "$DATA_DIR")"
_dd_real="$(cd "$(dirname "$DATA_DIR")" && pwd -P)/$(basename "$DATA_DIR")"
_real_parent="$(cd "$(dirname "$REAL_DIR")" && pwd -P)"
_real_real="$_real_parent/$(basename "$REAL_DIR")"
  ...
  for asset in bin conda runtime seed-assets; do
    if [[ -d "$REAL_DIR/$asset" ]]; then
      if [[ "$(uname -s)" == "Darwin" ]]; then
        ...
        cp -R --reflink=auto "$REAL_DIR/$asset" "$DATA_DIR/$asset" 2>/dev/null || cp -R "$REAL_DIR/$asset" "$DATA_DIR/$asset"
      fi
    fi
  done
  echo "运行时就绪。"
